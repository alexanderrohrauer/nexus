...
def get_import_task():
    ...
    if len(jobs) > 0:
        job_dtos = [ImportJob(job_id=ImportJobId(job.id), cursor=ImportCursor(**job.kwargs["cursor"]),
                              enabled=job.next_run_time is not None) for job in jobs]
        return ImportTask(
...
def create_import_task(dto: CreateImportTaskRequest):
    ...
    for job_id in [e.value for e in ImportJobId]:
        ...
        cursor = ImportCursor()
        args = {"n_batches": dto.n_batches, "keywords": dto.keywords, "cursor": cursor.model_dump(),
...
def update_import_task(dto: UpdateImportTaskRequest):
    for job_id in [e.value for e in ImportJobId]:
        ...
        args = {"n_batches": dto.n_batches, "keywords": dto.keywords, "cron_expr": dto.cron_expr,
                "cursor": found_job.kwargs["cursor"]}
        try:
            trigger = CronTrigger.from_crontab(dto.cron_expr)
        except ValueError:
            raise HTTPException(status_code=400, detail="Cron expression has wrong format!")
